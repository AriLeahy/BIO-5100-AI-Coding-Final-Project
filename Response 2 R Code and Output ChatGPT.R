...
# Calculate single value across the entirety of the turn for each turning parameter
avg_turning_rate <- mean(data$turning_rate_deg_per_sec, na.rm = TRUE)
avg_turning_radius <- mean(data$turning_radius_m, na.rm = TRUE)
avg_turning_velocity <- mean(data$turning_velocity_m_per_sec, na.rm = TRUE)
avg_centripetal_acceleration <- mean(data$centripetal_acceleration_m_per_sec2, na.rm = TRUE)
avg_g_force <- mean(data$g_force, na.rm = TRUE)

# Output the single values
print(paste("Single Turning Rate (degrees per second):", avg_turning_rate))
print(paste("Single Turning Radius (meters):", avg_turning_radius))
print(paste("Single Turning Velocity (meters per second):", avg_turning_velocity))
print(paste("Single Centripetal Acceleration (meters per second squared):", avg_centripetal_acceleration))
print(paste("Single g-force (multiples of gravitational force):", avg_g_force))
